...
def load_resnet_model(device):
    ...
    resnet.fc = nn.Identity()  # 去掉分类层，输出特征维度为 512
    resnet.eval()
    resnet.to(device)
    return resnet
...
def main():
    ...
    act_real = get_activations_from_dataloader(real_loader, model, device, limit=sample_count)
    print(f"Real activations shape: {act_real.shape}")

    # ---- Generated images ----
    print("\nExtracting generated images activations...")
    act_gen = get_activations_from_folder(gen_folder, transform, model, device,
